...
const playerWin = () => {
    ...
    if (grid[0] == X_text ) {
        ...
        if (grid[2] == X_text && grid[4] == X_text) {
    ...
    if (grid[0] == O_text) {
        ...
        if (grid[2] == O_text && grid[4] == O_text) {
...
const stopTurn = (winner)=>{
    ...
    WiningBox.children[0] = winner;

    boxes.forEach((box)=>{
        box.removeEventListener('click',handleInput)
    })
}
...
btn.addEventListener('click',()=>{
    ...
    boxes.forEach((box,index)=>{
        box.innerHTML = '';
        box.addEventListener('click',handleInput(box,index));
    })
})


//------------------ Event listener to input boxes data
const handleInput = (box,index)=>{
    console.log('check box id ',box.id)
    if (grid[index] == null) {
